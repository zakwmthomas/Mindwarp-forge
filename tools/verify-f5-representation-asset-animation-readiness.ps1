...
foreach($required in @('deterministic, data-only','front, side, and top integer','no filesystem, network, process','future Unity installation','genuinely untrusted executable content','neutral-articulation-fixture-v2','squared length `14400`','articulation control','without changing segment','prevents a pose judgment')){if(!(Get-Content $p7bBuiltinContractPath -Raw).Contains($required)){throw "P7b built-in viewport contract missing: $required"}}
...
foreach($required in @('artifact-reference-viewport-002','preserves both forearm segment lengths','silently rebound')){if(!$p7bStimulusContract.Contains($required)){throw "P7b v2 stimulus contract missing: $required"}}
...
foreach($required in @('neutral-articulation-fixture-v2','artifact-reference-viewport-002','validate_reference_fixture_semantics','reference limb segment is not the legible v2 length','reference limb length drifts across pose frames','articulation_control_changes_pose_without_changing_segment_length')){if(!$p7bBuiltinSources.Contains($required)){throw "P7b v2 fixture implementation missing: $required"}}
