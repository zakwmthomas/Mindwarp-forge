...
$ErrorActionPreference='Stop';$root=Split-Path -Parent $PSScriptRoot;. (Join-Path $PSScriptRoot 'g1-c5-successor-route.ps1');if([string]::IsNullOrWhiteSpace($ReceiptPath)){$ReceiptPath=Join-Path $root 'docs\canonical-system\G1_VERTICAL_CLOSEOUT_RECEIPT.json'}
