#!/usr/bin/env python3
"""Produce a C5 external execution result on a GitHub-hosted native runner."""
...
MANIFEST=ROOT/"tools/fixtures/c5-significance-scheduler-receipt/Cargo.toml"
...
def main():
    ...
    env=os.environ.copy();target_dir=Path(os.environ.get("RUNNER_TEMP","/tmp"))/"forge-c5-independent-target";env["CARGO_TARGET_DIR"]=str(target_dir);env["FORGE_ROOT"]=str(ROOT)
    ...
    exe=target_dir/"release"/("c5-significance-scheduler-receipt.exe" if os.name=="nt" else "c5-significance-scheduler-receipt")
    ...
    result={"schema_version":1,"protocol_id":req["protocol_id"],"request_id":req["request_id"],"request_sha256":claimed,"challenge":req["challenge"],"result_id":str(uuid.uuid4()),
      ...
      "executions":executions,"claims":{"evidence_only":True,"promotion_authority":False,"activate_c6":False,"repository_mutation":False,"independent_platform_execution_claimed":True}}
    ...
    print(f"C5 external result produced: {result['result_id']} {result['result_sha256']}")
