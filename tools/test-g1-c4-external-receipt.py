...
def fixtures():
    ...
    request={"schema_version":1,"protocol_id":"G1-C4-INDEPENDENT-PLATFORM-V1","request_id":"r","challenge":"c"*64,"created_utc":"2026-07-19T00:00:00Z",
      ...
      "allowed_platforms":[{"target":"x86_64-unknown-linux-gnu","os":"linux","architecture":"x86_64","pointer_width":64,"endian":"little"}],
    ...
    result={"schema_version":1,"protocol_id":request["protocol_id"],"request_id":"r","request_sha256":request["request_sha256"],"challenge":request["challenge"],"result_id":"x",
...
def reject(name,mutate,attested=True):
    ...
    try:v.validate(request,result,attested)
    except Exception:return
    raise AssertionError(f"hostile admitted: {name}")
def main():
    request,result=fixtures();assert v.validate(request,result,True)["classification"]=="independent_platform_execution"
    cases={
