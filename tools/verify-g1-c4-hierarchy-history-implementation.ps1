...
function Get-C4TreeManifestSha([string]$Commit){$treeLines=@(git -c core.quotePath=true ls-tree -r --full-tree $Commit);if($LASTEXITCODE-ne0-or!$treeLines.Count){throw 'C4 tracked-tree manifest is unavailable.'};$treeText=$treeLines-join"`n";$treeHasher=[Security.Cryptography.SHA256]::Create();try{([BitConverter]::ToString($treeHasher.ComputeHash([Text.Encoding]::UTF8.GetBytes($treeText))).Replace('-','').ToLowerInvariant())}finally{$treeHasher.Dispose()}}
