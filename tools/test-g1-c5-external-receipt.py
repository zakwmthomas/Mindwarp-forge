#!/usr/bin/env python3
"""Hostile fixtures for the authority-negative C5 external receipt importer."""
from __future__ import annotations
import base64, copy, hashlib, importlib.util, json, tempfile
from pathlib import Path

MODULE=Path(__file__).with_name("verify-g1-c5-external-receipt.py")
spec=importlib.util.spec_from_file_location("c5verify",MODULE);v=importlib.util.module_from_spec(spec);spec.loader.exec_module(v)
GENERATOR=Path(__file__).with_name("generate-g1-c5-external-request.py")
generator_spec=importlib.util.spec_from_file_location("c5generate",GENERATOR);generator=importlib.util.module_from_spec(generator_spec);generator_spec.loader.exec_module(generator)
def b64(raw): return base64.b64encode(raw).decode()
def hashed(value,name): value[name]=v.digest(value);return value
def fixtures():
    payload=b"c5-independent-payload";line=payload.hex().encode()+b"\n";empty=b""
    request={"schema_version":1,"protocol_id":"G1-C5-INDEPENDENT-PLATFORM-V1","request_id":"g1-c5-"+("c"*16),"challenge":"c"*64,"created_utc":"2026-07-19T00:00:00Z",
      ...
      "semantic":{"receipt_id":"G1-C5-SIGNIFICANCE-SCHEDULER","expected_sha256":hashlib.sha256(payload).hexdigest(),"encoding":"lowercase_hex","max_decoded_bytes":65536},
      ...
      "provenance_policy":{"repository":"x/y","workflow_path":".github/workflows/g1-c5-independent-platform.yml","signer_workflow":"x/y/.github/workflows/g1-c5-independent-platform.yml"},
    ...
    execution=lambda n,p,start,end:{"ordinal":n,"launch_id":f"l{n}","pid":p,"started_utc":start,"ended_utc":end,"argv":["/tmp/target/c5-significance-scheduler-receipt","--start-at-unix-ms","1"],"exit_code":0,"stdout_base64":b64(line),"stdout_sha256":hashlib.sha256(line).hexdigest(),"stderr_base64":b64(empty),"stderr_sha256":hashlib.sha256(empty).hexdigest(),"executable_sha256":"f"*64}
    result={"schema_version":1,"protocol_id":request["protocol_id"],"request_id":request["request_id"],"request_sha256":request["request_sha256"],"challenge":request["challenge"],"result_id":"x",
      ...
      "provenance":{"kind":"provider_hosted_ci","provider":"github-actions","hosted_runner":True,"run_id":"1","run_attempt":"1","job_id":"j","job_url":"https://github/x/actions/runs/1","repository":"x/y","workflow_ref":"x/y/.github/workflows/g1-c5-independent-platform.yml@refs/heads/main","runner_image":"ubuntu24","attestation_required":True},
      ...
      "build":{"argv":["cargo","build","--release","--locked","--offline","--manifest-path","/repo/tools/fixtures/c5-significance-scheduler-receipt/Cargo.toml"],"exit_code":0,"stdout_sha256":"0"*64,"stderr_sha256":"0"*64,"executable_sha256_before":"f"*64,"executable_sha256_after":"f"*64},
      "self_test":{"argv":["/tmp/target/c5-significance-scheduler-receipt","--self-test"],"exit_code":0,"stdout_base64":b64(b"C5 semantic receipt self-test passed: strict 38-field CBOR, 8 receipt hostiles, 10 pressure transcripts, 92-ID registry, and 10 authority-negative flags.\n"),"stdout_sha256":hashlib.sha256(b"C5 semantic receipt self-test passed: strict 38-field CBOR, 8 receipt hostiles, 10 pressure transcripts, 92-ID registry, and 10 authority-negative flags.\n").hexdigest(),"stderr_base64":b64(empty),"stderr_sha256":hashlib.sha256(empty).hexdigest()},
      "executions":[execution(1,11,"2026-07-19T00:00:00Z","2026-07-19T00:00:02Z"),execution(2,12,"2026-07-19T00:00:01Z","2026-07-19T00:00:03Z")],"claims":{"evidence_only":True,"promotion_authority":False,"activate_c6":False,"repository_mutation":False,"independent_platform_execution_claimed":True}}
...
def main():
    ...
    cases={
      ...
      "request.empty-challenge":lambda q,r:q.update(challenge="",request_id="g1-c5-"),
      "request.short-challenge":lambda q,r:q.update(challenge="c"*62,request_id="g1-c5-"+("c"*16)),
      "request.uppercase-challenge":lambda q,r:q.update(challenge="C"*64,request_id="g1-c5-"+("C"*16)),
      "request.nonhex-challenge":lambda q,r:q.update(challenge="z"*64,request_id="g1-c5-"+("z"*16)),
      "request.challenge-type":lambda q,r:q.update(challenge=7,request_id="g1-c5-7"),
      ...
      "receipt.schema-boolean":lambda q,r:q.update(schema_version=True),
      "receipt.build-exit-boolean":lambda q,r:r["build"].update(exit_code=False),
      "receipt.self-test-exit-boolean":lambda q,r:r["self_test"].update(exit_code=False),
      "receipt.ordinal-boolean":lambda q,r:r["executions"][0].update(ordinal=True),
      ...
      "authority.c6-flip":lambda q,r:r["claims"].update(activate_c6=True),
    ...
    workflow=(Path(__file__).resolve().parents[1]/".github/workflows/g1-c5-independent-platform.yml").read_text(encoding="utf-8")
    ...
    for script in ("verify-g1-c5-platform-observation-receipt.ps1",):
    ...
    local_verifier=(Path(__file__).with_name("verify-g1-c5-platform-observation-receipt.ps1")).read_text(encoding="utf-8")
    ...
    print(f"C5 external receipt importer verified: {len(cases)+1} hostile cases and pinned workflow controls pass.")
