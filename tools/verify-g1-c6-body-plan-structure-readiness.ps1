...
if(!(Test-G1C6BodyPlanStructureImplementationRoute -Checkpoint $checkpoint)){throw 'C6 body-plan implementation route is not exact.'}
$c6=@($program.items|Where-Object id -eq C6)
if($c6.Count-ne1-or$c6[0].state-ne'executing'-or$c6[0].status-ne'active'-or$c6[0].gate-ne'implementation'-or(@($c6[0].depends_on)-join ',')-ne'C4,C5'){throw 'C6 body-plan master-program route is not exact.'}
