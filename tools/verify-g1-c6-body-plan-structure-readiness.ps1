...
if(!$bodyPlanRoute-and!$identitySuccessorRoute){throw 'C6 body-plan implementation or exact recorded successor route is not exact.'}
$c6=@($program.items|Where-Object id -eq C6)
$expectedGate=if($bodyPlanRoute){'implementation'}else{'design'}
...
if($identitySuccessorRoute-and@($checkpoint.verification_receipts)-notcontains'receipt:G1-C6-BODY-PLAN-STRUCTURE-V1:recorded'){throw 'C6 body-plan recorded successor receipt missing.'}
