...
try{
 ...
 Expect-Failure 'C5 complete before successor' { $p=Get-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Raw|ConvertFrom-Json;$c=@($p.items|? id -eq C5)[0];$c.state='verified';$c.status='complete';$p|ConvertTo-Json -Depth 100|Set-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Encoding utf8 }
 Expect-Failure 'C6 activation' { $p=Get-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Raw|ConvertFrom-Json;$c=@($p.items|? id -eq C6)[0];$c.state='executing';$c.status='active';$p|ConvertTo-Json -Depth 100|Set-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Encoding utf8 }
 ...
 Expect-Failure 'successor manifest drift' { $p=Join-Path $temp 'docs/canonical-system/G1_C5_CLOSURE_RESULT.md';(Get-Content $p -Raw).Replace('e8e79816668d1f49167a895c068de61ad363784483534c184462066948eb1658',('f'*64))|Set-Content $p -Encoding utf8 }
