...
try{
 ...
 Expect-Failure 'C6 implementation gate downgraded' { $p=Get-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Raw|ConvertFrom-Json;$c=@($p.items|? id -eq C6)[0];$c.gate='design';$p|ConvertTo-Json -Depth 100|Set-Content (Join-Path $temp 'docs/canonical-system/MASTER_PROGRAM.json') -Encoding utf8 }
