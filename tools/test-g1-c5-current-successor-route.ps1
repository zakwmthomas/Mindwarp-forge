...
}elseif(Test-G1C6EcologicalNicheSemanticsDesignReadinessRoute -Checkpoint $canonical){
  Reject-C6Mutation 'ecology design/readiness source boundary omission' {$canonical.authority_lane=$value.Replace('No production crate or source, Cargo member or dependency, consumer implementation, ','')} {$canonical.authority_lane=$value}
}else{
