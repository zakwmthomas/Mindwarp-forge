...
if (!(Test-G1C5FullGateReconciliationRoute -Checkpoint $checkpoint)) {
    throw 'Canonical C5 full-gate reconciliation route was rejected.'
}
if ((& (Join-Path $PSScriptRoot 'test-c3-federated-interruption.ps1') -Checkpoint $checkpoint) -ne $true) {
    throw 'Canonical C5 full-gate reconciliation route was rejected by the interruption guard.'
}
...
function Reject-Mutation([string]$Label, [scriptblock]$Mutate, [scriptblock]$Restore) {
    ...
    try {
        if (Test-G1C5FullGateReconciliationRoute -Checkpoint $checkpoint) {
            throw "Forged C5 full-gate route was admitted: $Label"
        }
    } finally {
        & $Restore
    }
...
Reject-Mutation 'state' { $checkpoint.state = 'verified' } { $checkpoint.state = $value }
$value = $checkpoint.substage_id
Reject-Mutation 'substage' { $checkpoint.substage_id = 'c5-reconciliation-readiness' } { $checkpoint.substage_id = $value }
...
Write-Output 'C5 current successor route verified: exact batch, item, state, substage and authority pair fail closed.'
