...
$auditPath = Join-Path $root 'docs\canonical-system\G1_C3_POST_FIXED160_INTERVAL_CONSUMER_REASSESSMENT.md'
...
foreach ($required in @(
  'interval optical cell-step mathematical and oracle audit',
  'replayable point-box provenance',
  'interval bulk transfer',
  'do not infer implementation readiness',
  'generic physical passage',
  'already covered separately by the swept-AABB reference',
  'ambiguous_next_face',
  'No master obligation is newly closed',
  'Do not add a schema'
)) {
  if ($audit -notlike "*$required*") { throw "Post-fixed160 reassessment is missing: $required" }
}

$implementedRoute = $c3[0].next_action -like '*post-cell-step consumer reassessment*' -and
  $c3[0].next_action -like '*path composition*' -and
  $checkpoint.batch_id -eq 'G1-C3-POST-INTERVAL-CELL-STEP-CONSUMER-REASSESSMENT-V1' -and
  $checkpoint.next_action -like '*interval bulk transfer*' -and
  $checkpoint.next_action -like '*path composition*'
$preImplementationRoute = $c3[0].next_action -like '*interval cell-step*' -and
  $c3[0].next_action -like '*composer*' -and
  $checkpoint.batch_id -like 'G1-C3-INTERVAL-OPTICAL-CELL-STEP-*' -and
  $checkpoint.next_action -like '*interval cell-step*'
$downstreamRoute = $c3[0].next_action -like '*fixed-interval arithmetic consolidation*' -and
  $c3[0].next_action -like '*Do not*migrate optical arithmetic*interval bulk*composition*' -and
...
$postConsolidationRoute = $c3[0].next_action -like '*post-consolidation consumer reassessment*' -and
  ...
  $c3[0].next_action -like '*Do not implement either route*' -and
  $checkpoint.batch_id -eq 'G1-C3-POST-FIXED-INTERVAL-ARITHMETIC-CONSOLIDATION-REASSESSMENT-V1'
$intervalBulkOwnerGate = $c3[0].next_action -like '*G1_C3_INTERVAL_BULK_TRANSFER_IMPLEMENTATION_READINESS.md*' -and
  $c3[0].next_action -like '*explicit owner approval*' -and
  $c3[0].proof -like '*Optical migration supplies no prerequisite*' -and
  $c3[0].proof -like '*No source*composition*C3-closure claim*' -and
...
$calibratedBasisGapRoute = $c3[0].next_action -like '*calibrated-basis*transport-applicability schema gap audit*' -and
  $checkpoint.batch_id -eq 'G1-C3-CALIBRATED-SPECTRAL-TIME-BASIS-MATHEMATICAL-DESIGN-V1' -and
  $checkpoint.substage_id -eq 'calibrated-spectral-time-basis-oracle-result' -and
  $checkpoint.authority_lane -like '*Code-free mathematical design and deterministic exact-rational oracle only*No crate*contract schema*production source*normative wavelength*tick duration*spatial scale*visibility*promotion*C3 closure*'
$federatedContinuityRoute = & (Join-Path $PSScriptRoot 'test-c3-federated-interruption.ps1') -Checkpoint $checkpoint -C3 $c3[0]
if (-not ($implementedRoute -or $preImplementationRoute -or $downstreamRoute -or $postConsolidationRoute -or $intervalBulkOwnerGate -or $intervalBulkImplementation -or $postIntervalBulkRoute -or $lineageOwnerGate -or $lineageImplementation -or $postLineageRoute -or $cumulativeOwnerRoute -or $cumulativeImplementationRoute -or $receiverDesignRoute -or $receiverOwnerRoute -or $receiverImplementationRoute -or $phaseSpaceReadinessRoute -or $transportOwnerRoute -or $transportImplementationRoute -or $calibratedBasisGapRoute -or $federatedContinuityRoute)) {
  throw 'C3 does not retain the cell-step-before-bulk ordering and composition stop condition.'
}
if ($c3[0].next_action -like '*G1_C3_INTERVAL_INCIDENT_FIXED160_IMPLEMENTATION_READINESS.md*' -or
    $checkpoint.batch_id -eq 'G1-C3-INTERVAL-INCIDENT-FIXED160-OWNER-GATE-V1') {
  throw 'Stale pre-implementation interval wording returned to C3.'
}

Write-Output 'Post-fixed160 consumer reassessment verified: cell-step precedes interval bulk and composer work remains blocked.'
