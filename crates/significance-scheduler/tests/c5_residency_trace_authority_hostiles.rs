...
fn trace() -> (Vec<WorkTicket>, PressureTraceV2) {
    let tickets = vec![
        ticket(1, ConsumerDomainV1::Generation),
        ticket(2, ConsumerDomainV1::Simulation),
    ];
    let mut scheduler = ReferenceScheduler::new(tickets.clone(), budget()).unwrap();
...
fn trace_packet_mismatch() {
    let (tickets, trace) = trace();
    let mut hostile = trace.clone();
    hostile.decisions[0].packet_fingerprint = [8; 32];
    assert!(hostile.verify_replay(&tickets, budget()).is_err())
}
#[test]
fn trace_budget_mismatch() {
    let (tickets, trace) = trace();
    let other = BudgetEnvelope::new(1, [0, 7, 0, 0], [0; 4], 3).unwrap();
    assert!(trace.verify_replay(&tickets, other).is_err())
}
#[test]
...
fn trace_replay_drift() {
    let (mut tickets, trace) = trace();
    tickets[0].cost_units = 2;
    assert!(trace.verify_replay(&tickets, budget()).is_err());
    trace
        .verify_replay(
            &[
                ticket(1, ConsumerDomainV1::Generation),
