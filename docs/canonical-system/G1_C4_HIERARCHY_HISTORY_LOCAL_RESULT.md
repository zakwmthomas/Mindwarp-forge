# G1 C4 hierarchy/history local verification result

Status: **candidate_verified_local; not promoted.**

Broad C4's additive capability-free implementation is locally verified against
the accepted exact C2 plus promoted C3A boundary. It does not activate C5 and
does not grant runtime, storage, C3B, ecology, presentation, filesystem,
network, multiplayer, Companion, Greenfield or Kernel authority.

## Implemented evidence

- stable domain-separated dynamic instance identity and strict never-observed,
  absent, present and tombstoned evidence;
- retained ambient cohort binding across entity, assignment contract and exact
  cohort, with no population distribution or species-weight claim;
- exact dependency availability, typed semantic known-good-prefix recovery,
  unsupported topology refusal and atomic one/two-hop migration validation;
- canonical window, full-stream and snapshot bytes with pinned deterministic
  window rows `0/1/16/256` and history rows `0/1/16/64/256`;
- a strict deterministic 27-field C4 semantic receipt built by replaying the
  real fixed C3A input and packet through `addressable-world-binding`;
- executable ownership for all 74 frozen hostile IDs: 58 core Rust cases,
  eight receipt mutations and eight portability classification guards.

The semantic receipt SHA-256 is
`263a7c274c5bbfb5a48f0a7ccf3462eb35ddc7c96c1c92ff01d8ef37a40f6996`.
The bounded nine-file implementation source manifest SHA-256 is
`b407b6e82c248cded8c65838d4cef961c21d51d05b1a00ea75d2318756ebb1bf`.
The retained platform observations bind clean source commit
`3aa3f5b07ccce0e80d8c1461244e671907c450cb` and complete tracked-tree
manifest SHA-256
`70b5f171daf4b35359306df6b959d0f422d6756a3f430896b66f00bd9c1d08b9`.
Two separately launched native Windows x64 processes emitted identical bytes.
The same bytes also executed on Windows i686 and are classified only as
`same_host_second_architecture`. The receipt and dependencies compiled for
`aarch64-linux-android`, classified only as `compile_only`.

## Verification

`tools/verify-g1-c4-hierarchy-history-implementation.ps1` passed formatting,
focused warning-free clippy for both changed crates, 43 focused component and
C4 hostile tests, seven retained C4V tests, exact C2/C3A semantic replay, two
fresh native processes, same-host i686 execution and Android compilation.
Exact commands, process IDs, separate stdout/stderr hashes, executable hashes,
toolchains, target metadata and typed absence are retained in
`G1_C4_LOCAL_PLATFORM_OBSERVATIONS.json`.

The verifier is registered in the full Forge gate. A registered full-gate run
has not been recorded for this candidate because the required independent
second-platform execution receipt is still absent.

## Remaining gate

Windows x64 and i686 on this host do not prove independent execution. Android
compilation does not prove execution. C4 therefore stops at
`candidate_verified_local` until the identical semantic receipt executes on a
genuinely independent, platform-diverse runner. No elapsed time, same-host
architecture, compile-only target or same-platform remote Windows x64 runner
may be substituted for that evidence.

Deletion of the new additive APIs, tests, receipt fixture, verifier and this
result record is the rollback boundary. The legacy one-hop migration content
ID is pinned as `ea19d3e5369ec0e1afbbcd1717e2dcf45393e0113276b541628846959c65676e`;
existing C3A/C4V evidence remains unchanged.
