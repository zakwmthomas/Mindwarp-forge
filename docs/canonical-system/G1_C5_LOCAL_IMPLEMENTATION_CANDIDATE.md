...
Status: **local hardened full-hostile and pressure candidate; independent re-review accepted portability only; not C5 closure.**
...
Portability in this package applies only to that capability-free game-facing proof contract. It does not port Forge itself: Forge remains the PC-first tool/factory that produces and verifies the game.
...
The local suite now directly exercises every frozen hostile ID, end-to-end verified packet-derived truth through admission and replay, all ten pressure scenarios, stale-route fallback quarantine, strict rejection receipts, and disposable residency request/renew/expiry/bypass/churn behavior. The first independent review correctly blocked portability and caused these hardening changes; a fresh independent re-review then accepted advancement into portability only. Portable sealed receipt execution, fresh-process/i686/Android/hosted evidence, read-only ProofReceipt integration and the registered full Forge gate remain required before C5 closure or C6 activation.

The isolated portability fixture now seals a strict 38-field semantic receipt containing the eight domain rows, ten deterministic pressure transcripts, the exact 92-ID hostile registry and ten authority-negative flags. The expanded local verifier reproduces semantic SHA-256 `88e2be61586e728613fe2c7bf5b947074459fc5f63d6e5f13d4f4648e64624eb` across two fresh Windows x64 processes and same-host Windows i686 execution, and compiles the same fixture for Android ARM64 without claiming execution there. These are local observations only; the clean-commit observation record and genuinely independent hosted execution remain pending.
