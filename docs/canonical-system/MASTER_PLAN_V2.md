# Mindwarp Forge Master Plan v2

Status: canonical planning doctrine. The machine-selectable execution truth is
`MASTER_PROGRAM.json`; this document defines how that registry is interpreted.
No paragraph here grants approval, promotion, spending, installation,
publishing, credential, runtime, or protected-Kernel authority.
...
1. Evidence precedes trust. Recovered and inherited artifacts are quarantined
   inputs until their fixity, provenance, behavior, and failure boundaries pass
   disposable verification.
2. References are evidence, tests, and candidate guidance; they are never
   silently promoted to canonical baselines or implementation authority.
3. AI output is reviewed candidate input. Humans retain production-code,
   promotion, spending, publishing, credential, and runtime authority.
4. One canonical execution registry selects work. Narrative plans explain;
   generated views present; neither may become a second queue.
5. Work advances in meaningful, dependency-ordered packages with an owner gate
   after every material package. Heartbeat automation remains paused throughout
   rebaseline unless the owner explicitly resumes it.
6. Testing is layered: static contract checks, focused unit/adversarial tests,
   integration and authority-negative tests, disposable recovery/replay tests,
   then the full Forge gate. A passing narrow test cannot overrule a failing
   broader gate.
7. Similarity metrics must be calibrated against known controls before any
   numeric acceptance threshold is authorized. No provisional 90/95 rule is an
   acceptance gate.
...
Every registry item has one canonical `state`:
`proposed`, `researched`, `designed`, `ready_for_owner`, `authorized`,
`executing`, `blocked`, `verified`, `promoted`, `superseded`, `rejected`, or
`retired`. Exactly one item may be `executing`. `verified` means its declared
proof passed; `promoted` additionally means the result is the retained canonical
choice. Owner gates cannot be inferred from worker wakes, test success, or
generated views.

Schema v2 temporarily emits the old `status` field as a compatibility
projection for existing readers. `verify-master-program.ps1` requires exact
...
annotated tag before repair. Completed by checkpoint
`3e97c8b` / `checkpoint/reference-guided-wip-2026-07-14`.

R0.2 Install registry schema v2, lifecycle validation, explicit provenance
routing, this doctrine, a coverage classification for older plans, and a
generated owner-facing master-plan view.

R0.3 Repair the incomplete v3 neutral T-pose fixture as one internally
consistent package: identifiers, joint vocabulary, control geometry,
contracts, verifiers, stimulus binding, formatting, focused tests, and full
gate. Retain old owner receipts against their original fingerprints; never
rebind them to v3.

R0.4 Close the risk-critical context-compiler continuity gap with a
representative long-corpus fixture, deterministic replay, migration proof,
Windows path-boundary tests, and explicit SQLite-handle closure.

R0.5 Run disposable recovery verification and the complete Forge gate. Produce
one evidence-backed owner review package listing passes, failures, unresolved
risks, rollback point, and proposed next authorization. Stop for owner review.

## Humanoid proof chain

H1 Source and reference intake: recovered plus synthetic references only at
first; fixity, provenance, license/authority, quarantine, and contradiction
records are mandatory.

H2 Representation contract: stable identity, hierarchy, rest/bind pose,
coordinate convention, named joints, topology limits, and deterministic
serialization.

H3 Forge generation: deterministic engine-neutral construction from declared
inputs, with no external executable content or hidden mutation.

H4 Functional controls: known broken-connection, silhouette-collapse, and
articulation-drift controls; calibration establishes what metrics can and
cannot distinguish.

H5 Inspection and observation: read-only Reference Studio presentation binds
the exact artifact fingerprint, frame, views, assertions, limitations, and
owner wording. Observation has no promotion effect.

H6 Reproduction and recovery: clean rebuild, replay, corruption recovery,
stable identifiers, and retained receipts prove continuity.

H7 Promotion gate: only after structural, functional, provenance, recovery,
and calibrated-observation evidence is complete may the owner authorize a
...
artifacts changed; tests and disposable checks run; failures and limitations;
