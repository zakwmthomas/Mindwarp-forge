...
| C4 | Hierarchy/history semantics | Residency/delta/migration/corruption tests | Verified against exact C2+C3A. All 74 frozen hostile IDs, deterministic scale/cost, local x64, same-host i686 and Android compile evidence pass; GitHub-hosted Linux execution and retained attestation replay bind source `f505c82`. Registered complete gate `run-bc2154f73f6243239910ac30bc3b1994` passed in 632411 ms. C3B, production storage and runtime remain excluded |
| C5 | Significance/scheduler | Pressure/cancellation/starvation/thrash tests | Active for owner-authorized reconciliation/readiness only: scheduler reference and four-domain fidelity fixture retained; complete named-domain dispatch, admission, budget, fallback, cancellation and fairness proof remains open |
...
- Broad C4 is verified and recorded after hosted Linux execution, retained replay and registered gate `run-bc2154f73f6243239910ac30bc3b1994`. C5 is the sole executing/active item for reconciliation/readiness only. GP0-GP4 and `G1-VERTICAL-CLOSEOUT` remain completed immutable evidence; C6-C7, broad `G1-CLOSEOUT` and R1 remain gated and unchanged.
