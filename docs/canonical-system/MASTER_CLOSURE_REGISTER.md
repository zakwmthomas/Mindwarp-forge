...
- Broad C4 depends on C2 and C3A only. GP0 is structurally verified without gameplay-system promotion; GP1 is independently and completely verified as a capability-free fixed-loop proof; GP2 typed progression is promoted after registered full-gate proof `run-2dc3db644adc416a8ef56461dbb771b6`. C4V is recorded after registered full-gate proof `run-fa6334a300e04d409dd5cddb4f22542e` with exact C2, C3A and GP1 dependencies, and remains the master cursor only while awaiting owner direction. GP3 remains queued, and GP4 depends exactly on GP3 plus C4V.
